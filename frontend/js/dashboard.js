...
// Show section with spinner and slide/fade animations
function showSectionWithSpinner(sectionId, event) {
  showSpinner();
  // Simulate data load delay (remove or adjust in production)
  setTimeout(() => {
    // Hide all sections with slide-out animation
    const sections = ['overviewSection', 'depositSection', 'sendMoneySection', 'transactionsSection', 'loansSection'];
    sections.forEach(id => {
    ...
    // After slide-out, show the target section with slide-in animation
    setTimeout(() => {
      const activeSection = document.getElementById(sectionId);
      if (activeSection) {
